...
The `handle` method always returns a list where every element represents a node in the heap. Every list element is again a list consisting of the node's `handle` and its `id`. So you need to check the node`s value to figure out the
