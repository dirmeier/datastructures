...
Sometimes, we want to decrease a key and but that rearrange its position in the
heap. If we have a *unique* key this can be easily done like that:
...
The function takes vectors, so you can also do it like this:
...
However, things get more complicated if we have multiple identical keys like we decreases above. In this case, in order to decrease to correct node, we need to get its `handle` first:

```{r}
  handle(fh, -15)
```

The `handle` methods returns the `id` of the node and the `value` that is stored
...
```{r}
  hand <- handle(fh, -15)
  decrease_key(fh, -15, hand[[1]]$handle)
```

The `handle` method always returns a list where every element represents a node in the heap. Every list element is again a list constisting of the node's `handle` and its `id`. So you need to check the node`s value to figure out the
corresponding `id`. (This is the easiest implementation I could think of. Better suggestions are welcome as using this is not straightforward I find)
